            ...
            <div class="col-lg-6">
                <div class="input-group input-group-sm">
                    ...
                    <input type="text" id="searchInput" class="form-control bg-light border-0 py-2"
                        placeholder="Cari surah, ayat, juz, atau tanggal...">
                </div>
            </div>
            <div class="col-lg-3">
...
<div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden">
                    ...
                    <?php if (!empty($riwayat) && is_array($riwayat)): ?>
                        ...
                        foreach ($riwayat as $row): ?>
                            <tr class="item-row"
                                ...
                                <td class="ps-4 fw-medium text-secondary nomor-urut"><?= $no++; ?></td>
                                <td>
                                    <small class="text-secondary" style="font-size: 0.8rem;"><i
                                            class="fa-regular fa-calendar me-1"></i>
                                        <?= date('d M Y, H:i', strtotime($row['created_at'])); ?> WIB</small>
                                </td>
                                <td>
                                    <?php if (strtolower($row['jenis']) == 'ziyadah'): ?>
                                        <span
                                            class="badge bg-success bg-opacity-10 text-success px-2 py-1 rounded-pill small fw-semibold">Ziyadah</span>
                                    <?php else: ?>
                                        <span
                                            class="badge bg-primary bg-opacity-10 text-primary px-2 py-1 rounded-pill small fw-semibold">Murojaah</span>
                                    <?php endif; ?>
                                    <small class="text-secondary d-block mt-1">Juz <?= esc($row['juz']); ?></small>
                                </td>
                                <td>
                                    <span class="fw-semibold text-dark-mode d-block">Surah <?= esc($row['surah']); ?></span>
                                    <small class="text-secondary">Ayat <?= esc($row['ayat_mulai'] ?? '-'); ?> -
                                        <?= esc($row['ayat_selesai'] ?? '-'); ?></small>
                                </td>
                                <td class="text-center">
                                    ...
                                    ?>
                                    <span
                                        class="badge <?= $badgeClass; ?> px-3 py-1 rounded-pill small fw-semibold"><?= esc($row['predikat']); ?></span>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr id="emptyRow">
                            <td colspan="6" class="text-center py-4 text-muted small">Belum ada data riwayat setoran hafalan
                                untuk santri ini.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
    ...
    <!-- Card Footer info -->
    <div class="card card-footer bg-white border-0 py-3 px-4 d-flex justify-content-between align-items-center">
        <span class="text-secondary small" id="infoJumlah">Menampilkan total
            <?= !empty($riwayat) ? count($riwayat) : 0; ?>
            riwayat setoran</span>
    </div>
</div>
