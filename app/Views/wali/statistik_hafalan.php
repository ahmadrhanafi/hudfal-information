...
<div class="container-fluid px-0">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
        <div>
            <h3 class="fw-bold text-dark-mode mb-1" style="text-transform: none !important;">Statistik Hafalan
                <?= esc($selected_santri['nama_santri'] ?? 'Ananda'); ?>
            </h3>
        ...
        <div class="d-flex align-items-center gap-2 flex-wrap">
            ...
            <?php if (!empty($santri) && is_array($santri) && count($santri) > 1): ?>
                <div class="dropdown">
                    <button class="btn btn-outline-success btn-sm dropdown-toggle rounded-pill px-3 py-2 bg-white shadow-sm"
                        type="button" id="dropdownPilihAnak" data-bs-toggle="dropdown" aria-expanded="false">
                        <i class="fa-solid fa-child-reaching me-1"></i>
                        <strong><?= esc($selected_santri['nama_santri']); ?></strong>
                    </button>
                    <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-4 p-2"
                        aria-labelledby="dropdownPilihAnak" style="min-width: 200px;">
                        <?php foreach ($santri as $a): ?>
                            <li>
                                <a class="dropdown-item rounded-3 py-2 px-3 small <?= ($a['id'] == $selected_santri['id']) ? 'active bg-success text-white' : 'text-dark'; ?>"
                                    ...
                                    <?= esc($a['nama_santri']); ?> <small
                                        class="opacity-75">(<?= esc($a['nama_kelas'] ?? '-'); ?>)</small>
                                </a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
            ...
            <form method="get" action="" id="formPeriode" class="d-flex align-items-center">
                ...
                <?php endif; ?>
                <select name="periode"
                    class="form-select form-select-sm bg-white border shadow-sm rounded-pill px-3 py-2"
                    style="width: 140px !important;" onchange="document.getElementById('formPeriode').submit()">
                    <option value="minggu_ini" <?= (($periode ?? '') == 'minggu_ini') ? 'selected' : ''; ?>>Minggu Ini
                    </option>
                    <option value="bulan_ini" <?= (($periode ?? '') == 'bulan_ini') ? 'selected' : ''; ?>>Bulan Ini
                    </option>
                    <option value="semester_ini" <?= (($periode ?? '') == 'semester_ini') ? 'selected' : ''; ?>>Semester
                        Ini</option>
                </select>
            </form>
        </div>
        ...
        <div class="col-xl-4 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 h-100">
                <div class="card-body p-4 d-flex align-items-center gap-3">
                    ...
                    <div>
                        <span class="text-secondary small fw-medium" style="text-transform: none !important;">TOTAL JUZ
                            SELESAI</span>
        ...
        <div class="col-xl-4 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 h-100">
                <div class="card-body p-4 d-flex align-items-center gap-3">
                    ...
                    <div>
                        <span class="text-secondary small fw-medium" style="text-transform: none !important;">STREAK
                            SETORAN</span>
                        <h3 class="fw-bold mb-0 mt-1 text-dark-mode">
        ...
        <div class="col-xl-4 col-md-12">
            <div class="card border-0 shadow-sm rounded-4 h-100">
                <div class="card-body p-4 d-flex align-items-center gap-3">
                    ...
                    <div>
                        <span class="text-secondary small fw-medium" style="text-transform: none !important;">PREDIKAT
                            RATA-RATA</span>
    ...
    <div class="row g-4 mb-4">
            ...
            <div class="card border-0 shadow-sm rounded-4 p-4 h-100">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="d-flex align-items-center gap-2">
                        ...
                        <h5 class="fw-bold mb-0 text-dark-mode"
                            style="text-transform: none !important; font-size: 1.05rem;">Grafik Setor Hafalan Al-Qur'an
                        </h5>
                    </div>
                    <span
                        class="badge bg-secondary-subtle text-secondary border px-3 py-2 rounded-pill small fw-semibold">
                        <?= esc(ucwords(str_replace('_', ' ', $periode ?? 'bulan_ini'))); ?>
                    </span>
                </div>
                <p class="text-secondary small mb-4">Grafik tren pencapaian setoran hafalan berdasarkan rentang waktu
                    terpilih.</p>
        ...
        <div class="col-xl-4">
            <div class="card border-0 shadow-sm rounded-4 p-4 h-100">
                ...
                <p class="text-secondary small mb-4">Persentase rasio perbandingan antara hafalan baru dan pengulangan.
                </p>
                ...
                <div class="mb-4">
                    ...
                    <div class="progress rounded-pill bg-secondary-subtle" style="height: 10px;">
                        <div class="progress-bar bg-success rounded-pill" role="progressbar"
                            style="width: <?= esc($komposisiZiy); ?>%;"></div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="progress rounded-pill bg-secondary-subtle" style="height: 10px;">
                        <div class="progress-bar bg-primary rounded-pill" role="progressbar"
                            style="width: <?= esc($komposisiMur); ?>%;"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden">
        <div class="card card-header bg-white border-0 pt-4 px-4 pb-0">
            <h5 class="fw-bold text-dark-mode mb-1" style="text-transform: none !important; font-size: 1.05rem;">
                Detail Capaian per Juz</h5>
            <p class="text-secondary small mb-3">Status penyelesaian juz Al-Qur'an ananda berdasarkan filter terpilih.
            </p>
        </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light text-muted small text-uppercase"
                        ...
                        <tr>
                            ...
                            <th class="py-3" style="width: 12%;">Juz</th>
                            <th class="py-3" style="width: 33%;">Nama Surah / Keterangan</th>
                            <th class="py-3" style="width: 20%;">Total Setoran</th>
                    ...
                    <tbody>
                        <?php if (!empty($detailJuz) && is_array($detailJuz)): ?>
                            <?php foreach ($detailJuz as $row): ?>
                                ...
                                $rJenis = is_array($row) ? (string) ($row['jenis'] ?? '-') : '-';
                                $rTotal = is_array($row) ? (string) ($row['total_setoran'] ?? '1') : '1';
                                $rPred = is_array($row) ? (string) ($row['predikat'] ?? '-') : '-';

                                // Menangkap tanggal setoran dari database
                                $rTgl = $row['tanggal'] ?? $row['created_at'] ?? null;
                                ...
                                ?>
                                <tr>
                                    <td class="ps-4 text-dark small fw-medium">
                                        <?php if (!empty($rTgl)): ?>
                                            <span class="d-block text-dark-mode fw-semibold" style="font-size: 0.8rem;">
                                                <i class="fa-regular fa-calendar-days text-secondary me-1"></i>
                                                <?= date('d M Y', strtotime($rTgl)); ?>
                                            </span>
                                            <small class="text-secondary" style="font-size: 0.7rem;">
                                                <i class="fa-regular fa-clock me-1"></i>
                                                <?= date('H:i', strtotime($rTgl)); ?> WIB
                                            </small>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <span class="fw-semibold text-dark-mode d-block">Juz <?= esc($rJuz); ?></span>
                                        <?php if (!empty($rJenis) && $rJenis !== '-'): ?>
                                            <?php
                                            $jenisLower = strtolower($rJenis);
                                            $badgeJenis = 'bg-info bg-opacity-10 text-info';
                                            if (str_contains($jenisLower, 'ziyadah')) {
                                                $badgeJenis = 'bg-success bg-opacity-10 text-success';
                                            } elseif (str_contains($jenisLower, 'murojaah')) {
                                                $badgeJenis = 'bg-primary bg-opacity-10 text-primary';
                                            }
                                            ?>
                                            <span class="badge <?= $badgeJenis; ?> border-0 px-2 py-0 mt-1"
                                    ...
                                    <td>
                                        <span class="fw-semibold text-dark-mode d-block small"><?= esc($rSurah); ?></span>
                                    ...
                                    <td>
                                        <span class="badge bg-light text-dark border px-2 py-1"><?= esc($rTotal); ?> Kali
                                            Setor</span>
                                    </td>
                                    <td class="text-end pe-4">
                                        <span
                                            class="badge <?= $badgeClass; ?> px-3 py-1 rounded-pill small fw-semibold"><?= esc($rPred); ?></span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="5" class="text-center py-4 text-muted">Belum ada data capaian juz pada periode
                                    ini.
                                </td>
                            </tr>
                        <?php endif; ?>
        ...
        <div class="card card-footer bg-white border-0 py-3 px-4">
            <small class="text-secondary"><i class="fa-solid fa-circle-info me-1"></i> Data statistik diperbarui secara
                otomatis setiap kali ustadz pembimbing memasukkan log setoran harian.</small>
        </div>
    </div>
