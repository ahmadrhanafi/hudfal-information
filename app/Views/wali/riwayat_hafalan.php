...
<div class="container-fluid px-0">
        ...
        <div class="col-xl-4 col-md-6">
            <div class="card border-0 shadow-sm rounded-4 bg-white h-100">
                <div class="card-body p-4 d-flex align-items-center gap-3">
                    ...
                    <div>
                        <span class="text-secondary small fw-medium" style="text-transform: none !important;">JUZ AKTIF
                            SAAT INI</span>
                ...
                <div class="card-body p-4 d-flex align-items-center gap-3">
                    ...
                    <div>
                        <span class="text-secondary small fw-medium" style="text-transform: none !important;">TOTAL
                            SETORAN PERIODE INI</span>
                        <h3 class="fw-bold text-dark-mode mb-0 mt-1">
                            <?= esc($total_setoran ?? 0); ?> <span class="fs-6 fw-normal text-secondary">Kali
                                Setor</span>
                        </h3>
                    </div>
                </div>
            ...
            <form method="get" action="" id="formFilterRiwayat" class="row g-3 align-items-center">
                ...
                <div class="col-lg-<?= (!empty($santri_list) && count($santri_list) > 1) ? '4' : '7'; ?>">
                    <div class="input-group input-group-sm">
                        ...
                        <input type="text" id="searchRiwayat" class="form-control bg-light border-0 py-2"
                            placeholder="Cari berdasarkan surah, catatan, atau penguji...">
                    </div>
                </div>
                ...
                <?php if (!empty($santri_list) && count($santri_list) > 1): ?>
                    <div class="col-lg-4">
                        <select name="id_santri" class="form-select form-select-sm bg-light border-0 py-2"
                ...
                <div class="col-lg-<?= (!empty($santri_list) && count($santri_list) > 1) ? '4' : '5'; ?>">
                    <select name="periode" class="form-select form-select-sm bg-light border-0 py-2"
                        onchange="this.form.submit()">
                        <option value="bulan_ini" <?= (($periode ?? 'bulan_ini') == 'bulan_ini') ? 'selected' : ''; ?>>
                            Periode: Bulan Ini</option>
                        <option value="minggu_ini" <?= (($periode ?? '') == 'minggu_ini') ? 'selected' : ''; ?>>Periode:
                            Minggu Ini</option>
                        <option value="semester_ini" <?= (($periode ?? '') == 'semester_ini') ? 'selected' : ''; ?>>
                            Periode: Semester Ini</option>
                        <option value="semua" <?= (($periode ?? '') == 'semua') ? 'selected' : ''; ?>>Semua Riwayat
                        </option>
                    </select>
                </div>
            </form>
    ...
    <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="fw-semibold text-dark-mode d-block">Surah <?= esc($row['surah']); ?> <span
                                                class="badge bg-secondary rounded-lg text-white small"><?= esc($row['jenis']); ?></span></span>
                                        <small class="text-secondary">Juz <?= esc($row['juz']); ?> (Ayat
                                            <?= esc($row['ayat_mulai']); ?> - <?= esc($row['ayat_selesai']); ?>)</small>
                                    </td>
                                    <td>
                                        <span
                                            class="small text-dark-mode d-block"><?= esc($row['nama_guru'] ?? 'Ustadz Pembimbing'); ?></span>
                                    ...
                                    <td class="text-center">
                                        ...
                                        if (str_contains($predikat, 'jayyid jiddan') || str_contains($predikat, 'mumtaz'))
                                            $badgeClass = 'bg-primary bg-opacity-10 text-primary';
                                        elseif (str_contains($predikat, 'jayyid'))
                                            $badgeClass = 'bg-warning bg-opacity-10 text-warning text-dark';
                                        ?>
                                        <span
                                            class="badge <?= $badgeClass; ?> px-3 py-1 rounded-pill small fw-semibold mb-1"><?= esc($row['predikat']); ?></span>
                                        <small class="d-block text-secondary"
                                            style="font-size: 0.7rem;"><?= esc($row['catatan'] ?? 'Lancar'); ?></small>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr id="emptyRow">
                                <td colspan="5" class="text-center py-4 text-muted small">Belum ada riwayat setoran hafalan
                                    untuk anak Anda.</td>
                            </tr>
                        <?php endif; ?>
                    </tbody>
        ...
        <div class="card card-footer border-0 py-3 px-4 d-flex justify-content-between">
            <span class="text-secondary small">Menampilkan total <span
                    id="jumlahBaris"><?= !empty($riwayat) ? count($riwayat) : 0; ?></span> riwayat setoran</span>
        </div>
    </div>
